...
- **Nivel 3 (`pendiente_nivel3`):** el cafetero mandó su análisis.
  1. Copiar del chat los datos del análisis (pH, Al, Ca, Mg, K, P, S, B, Zn, Cu, Mn, textura).
  2. Armar un `lead.json` con los campos del formulario **+** `"servicio": "nivel_3"` y
     `"analisis_suelo": { ... }`. Plantilla en `ferticafe-motor/examples/lead_nivel3_demo.json`.
  3. `cd proyectos/ferticafe-motor && python operador.py --lead lead.json --pdf`
  4. Mandar el PDF de `out/` y pegar el mensaje que imprime la consola, en el chat del cafetero.
- **Si algo falla:** el servicio le avisa al cafetero "tuvimos un problema, ya quedó
  anotado". Revisar los logs en Render y reprocesar con `operador.py`.
